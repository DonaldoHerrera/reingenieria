package defpackage;

/* renamed from: bwa reason: default package and case insensitive filesystem */
/* compiled from: LocalSearchSuggestionOperations.kt */
final class C5676bwa extends C7540vYa implements C6308dYa<C3509cea, a, C4882Eva> {
    final /* synthetic */ C6884lwa a;

    C5676bwa(C6884lwa lwa) {
        this.a = lwa;
        super(2);
    }

    /* renamed from: a */
    public final C4882Eva invoke(C3509cea cea, a aVar) {
        C7471uYa.b(cea, "track");
        C7471uYa.b(aVar, "kind");
        return this.a.b(cea, aVar);
    }
}
