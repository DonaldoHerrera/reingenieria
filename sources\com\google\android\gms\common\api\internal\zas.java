...
final class zas implements zabs {
    ...
    private zas(Context context, zaaw zaaw, Lock lock, Looper looper, GoogleApiAvailabilityLight googleApiAvailabilityLight, Map<AnyClientKey<?>, Client> map, Map<AnyClientKey<?>, Client> map2, ClientSettings clientSettings, AbstractClientBuilder<? extends Oy, Ky> abstractClientBuilder, Client client, ArrayList<zaq> arrayList, ArrayList<zaq> arrayList2, Map<Api<?>, Boolean> map3, Map<Api<?>, Boolean> map4) {
        ...
        for (AnyClientKey put2 : map.keySet()) {
            t.put(put2, this.zaeg);
        }
        this.zaeh = Collections.unmodifiableMap(t);
    }

    public static zas zaa(Context context, zaaw zaaw, Lock lock, Looper looper, GoogleApiAvailabilityLight googleApiAvailabilityLight, Map<AnyClientKey<?>, Client> map, ClientSettings clientSettings, Map<Api<?>, Boolean> map2, AbstractClientBuilder<? extends Oy, Ky> abstractClientBuilder, ArrayList<zaq> arrayList) {
        ...
        for (Entry entry : map.entrySet()) {
            ...
            if (client2.requiresSignIn()) {
                t.put((AnyClientKey) entry.getKey(), client2);
            } else {
                t2.put((AnyClientKey) entry.getKey(), client2);
            }
        }
        Preconditions.checkState(!t.isEmpty(), "CompositeGoogleApiClient should not be used without any APIs that require sign-in.");
        T t3 = new T();
        T t4 = new T();
        for (Api api : map2.keySet()) {
            AnyClientKey clientKey = api.getClientKey();
            if (t.containsKey(clientKey)) {
                t3.put(api, (Boolean) map3.get(api));
            } else if (t2.containsKey(clientKey)) {
                t4.put(api, (Boolean) map3.get(api));
            } else {
        ...
        while (i < size) {
            ...
            i++;
            zaq zaq = (zaq) obj;
            if (t3.containsKey(zaq.mApi)) {
                arrayList2.add(zaq);
            } else if (t4.containsKey(zaq.mApi)) {
                arrayList3.add(zaq);
            } else {
                throw new IllegalStateException("Each ClientCallbacks must have a corresponding API in the isOptionalMap");
            }
        }
        zas zas = new zas(context, zaaw, lock, looper, googleApiAvailabilityLight, t, t2, clientSettings, abstractClientBuilder, client, arrayList2, arrayList3, t3, t4);
        return zas;
    }
    ...
    public final void maybeSignOut() {
        ...
        try {
            ...
            if (isConnecting) {
                new C1764us(this.zabj).post(new zat(this));
            } else {
                zay();
            }
        } finally {
